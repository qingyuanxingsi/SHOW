...
class Masking(nn.Module):
    def __init__(self):
        ...
        self.v_template = to_tensor(to_np(flame_model.v_template))
        self.set_bs(1)

    def set_bs(self,bs):
        # must be call before "to(device)" 
        vertices=self.v_template.repeat(bs,1,1).to(self.dtype)
        self.register_buffer('vertices', vertices)
        
    def to_render_mask(self, mask):
        face_mask = torch.zeros_like(self.vertices)
        if face_mask.ndimension()==2:
    ...
    def get_mask_rendering(self):
        face_mask = torch.ones_like(self.vertices)
        if face_mask.ndimension()==2:
            face_mask=face_mask[None]
        face_mask[:, self.masks.boundary, :] = 0.0
        face_mask[:, self.masks.neck, :] = 0.0
