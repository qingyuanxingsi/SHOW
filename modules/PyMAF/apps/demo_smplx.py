...
from pymaf_core.cfgs import cfg, parse_args
from pymaf_core import path_config, constants
# from cfgs import cfg, parse_args
# import path_config, constants
...
def run_demo(args):
    ...
    if 1:
        ...
        for key in tracking_results.keys():
            p=key.split('_')[-1]
            f=key.split('_')[-2]
            f_dict[f].append(p)
            p_dict[f][p]=key
            # if p in f_dict[f]:
        for f,p_list in f_dict.items():
            max_x=None
            if len(p_list)>1:
                for p in p_list:
                    key=p_dict[f][p]
                    det_face_kps=tracking_results[key]['joints2d_face'][0]
                    face_mean_x=np.mean(det_face_kps,axis=0)[0]

                    if max_x is None or face_mean_x>max_x:
                        max_x=face_mean_x
                        
                    if max_x is not None and face_mean_x<max_x:
                        print(f'{face_mean_x}<{max_x}')
                        tracking_results.pop(key)
    ...
    if args.recon_result_file:
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(dataloader):
                ...
                # mesh_out=preds_dict['mesh_out']
                # print(f'mesh_out length: {len(mesh_out)}')
        ...
        joblib.dump(to_save_results, os.path.join(output_path, "output.pkl"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_folder', type=str, 
                        default=r'C:\Users\lithiumice\code\speech2gesture_dataset\crop\seth\test_video\1-00_00_00-00_00_01\image',
                        help='input image folder')
    parser.add_argument('--output_folder', type=str, 
                        default=r'C:\Users\lithiumice\code\speech2gesture_dataset\crop\seth\test_video\1-00_00_00-00_00_01\pymaf',
    ...
    parser.add_argument('--cfg_file', type=str, 
                        default='configs/pymafx_config.yaml',
                        help='config file path.')
    parser.add_argument('--pretrained_model',
                        default='../data/pretrained_model/PyMAF-X_model_checkpoint.pt',
    ...
    parser.add_argument('--misc', 
                        default=['TRAIN.BHF_MODE', 'full_body', 'MODEL.EVAL_MODE', 'True', 'MODEL.PyMAF.HAND_VIS_TH', '0.1'], 
                        type=str, 
                        nargs="*",
